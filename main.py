...
def clasificar_saldos(saldos):
    ...
    medios = {nombre: saldo for nombre, saldo in saldos.items() if 200000 <= saldo < 350000}
    altos = {nombre: saldo for nombre, saldo in saldos.items() if saldo >= 350000}
...
def main():
    nombres = ['Ana', 'Luis', 'Marta', 'Pedro', 'Juan', 'Laura', 'Sofia', 'Carlos', 'Andres', 'Maria']
    saldos = {}
    while True:
        mostrar_menu()
